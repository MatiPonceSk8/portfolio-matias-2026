...
const Contact = ({ language, setLanguage }) => {
  ...
  const mainContentRef = useRef(null); // Referencia para aplicar el BLUR
  ...
  // --- 2. ANIMACIÓN DE "THANK YOU" (Se dispara al enviar) ---
  useEffect(() => {
    if (isSubmitted && window.anime && thankYouRef.current) {
        // Aseguramos que sea visible antes de animar
        ...
        tl.add({
            ...
            translateY: ['100%', '0%'], // Efecto de deslizar hacia arriba (tipo HT)
            opacity: [0, 1],
            duration: 1200,
            delay: window.anime.stagger(200) // Retraso entre líneas para elegancia
        })
        .add({
  ...
  }, [isSubmitted]); // Se ejecuta solo cuando isSubmitted cambia a true
  ...
  const handleSubmit = (e) => {
    ...
    // --- SIMULACIÓN DE ÉXITO ---
    console.log("Formulario enviado. Token:", token);
    
    // Activamos el modo "Thank You"
    setIsSubmitted(true); 
    
    // Scrollear arriba suavemente para centrar el mensaje
  ...
  return (
    <div ref={containerRef} style={{ minHeight: '200vh', backgroundColor: '#0B1120', position: 'relative', color: textColor, transition: 'background-color 0.1s linear' }}>
      ...
      {/* ========================================
        CONTENEDOR PRINCIPAL (SE DESENFOCA)
        ========================================
      */}
      <div ref={mainContentRef} style={{ 
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', padding: '40px 60px', display: 'flex', justifyContent: 'space-between', zIndex: 50, boxSizing: 'border-box' }}>
            <div onClick={() => !isSubmitted && navigate('/')} style={{ cursor: 'pointer', fontFamily: '"Inter", sans-serif', fontSize: '0.85rem', letterSpacing: '1px', opacity: 0.8, color: textColor }}>
                {t.back}
            </div>
        ...
        <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', position: 'relative', zIndex: 10 }}>
            <div ref={titleSectionRef} style={{ textAlign: 'center' }}>
                <h1 style={{ fontFamily: '"Playfair Display", serif', fontSize: '3.5rem', fontWeight: '400', margin: '0 0 20px 0', color: textColor, fontStyle: 'italic', letterSpacing: '-1px' }}>
                    {t.intro}
                </h1>
                <p style={{ fontFamily: '"Inter", sans-serif', fontSize: '0.9rem', color: accentColor, letterSpacing: '2px', textTransform: 'uppercase', opacity: 0.8 }}>
                    {t.subIntro}
                </p>
            </div>
        ...
        <div style={{ minHeight: '100vh', display: 'flex', justifyContent: 'center', alignItems: 'center', padding: '20px', position: 'relative', zIndex: 20 }}>
            <div style={{ maxWidth: '700px', width: '100%' }}>
                <form onSubmit={handleSubmit} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '40px' }}>
                    {/* Inputs */}
                    <div style={inputContainerStyle}><label style={labelStyle}>{t.name}</label><input type="text" required style={inputStyle} onFocus={handleFocus} onBlur={handleBlur} /></div>
                    <div style={inputContainerStyle}><label style={labelStyle}>{t.lastname}</label><input type="text" required style={inputStyle} onFocus={handleFocus} onBlur={handleBlur} /></div>
                    <div style={{...inputContainerStyle, gridColumn: '1 / -1'}}><label style={labelStyle}>{t.email}</label><input type="email" required style={inputStyle} onFocus={handleFocus} onBlur={handleBlur} /></div>
                    <div style={{...inputContainerStyle, gridColumn: '1 / -1'}}>
                        <label style={labelStyle}>{t.subject}</label>
                        <div style={{ position: 'relative' }}>
                            <select required style={{...inputStyle, cursor: 'pointer', appearance: 'none', borderRadius: 0, color: textColor}} onFocus={handleFocus} onBlur={handleBlur}>
                    ...
                    <div style={{...inputContainerStyle, gridColumn: '1 / -1'}}>
                        ...
                        <textarea required rows="1" style={{...inputStyle, resize: 'none', lineHeight: '1.5', overflow: 'hidden'}} onInput={(e) => {e.target.style.height = 'auto'; e.target.style.height = e.target.scrollHeight + 'px'}} onFocus={handleFocus} onBlur={handleBlur} />
                    </div>

                    {/* CLOUDFLARE TURNSTILE (INTEGRACIÓN SEGURA) */}
                    <div style={{ gridColumn: '1 / -1', marginBottom: '20px' }}>
                        <Turnstile 
                            siteKey={import.meta.env.VITE_CLOUDFLARE_SITE_KEY} // <-- AQUÍ ESTÁ EL CAMBIO
                            onSuccess={setToken} 
                            theme="dark" 
                    ...
                    <div style={{ gridColumn: '1 / -1', display: 'flex', justifyContent: 'flex-start' }}>
                        <button type="submit" style={{ background: 'transparent', color: accentColor, border: `1px solid ${accentColor}`, padding: '18px 50px', borderRadius: '0px', fontFamily: '"Inter", sans-serif', fontSize: '0.85rem', letterSpacing: '2px', cursor: 'pointer', textTransform: 'uppercase', transition: 'all 0.3s ease', opacity: token ? 1 : 0.5, pointerEvents: token ? 'auto' : 'none' }} onMouseEnter={(e) => { if(token) { e.target.style.background = accentColor; e.target.style.color = '#082019'; } }} onMouseLeave={(e) => { if(token) { e.target.style.background = 'transparent'; e.target.style.color = accentColor; } }}>
                            {t.send}
                        </button>
                    </div>
                </form>
      ...
      </div> {/* FIN DEL MAIN CONTENT REF */}


      {/* ========================================
        CAPA DE "THANK YOU" (OVERLAY)
        ========================================
      */}
      <div ref={thankYouRef} style={{ 
          ...
          <div style={{ overflow: 'hidden', marginBottom: '10px' }}>
            <h1 className="ty-text" style={{ fontFamily: '"Playfair Display", serif', fontSize: 'clamp(2rem, 5vw, 4rem)', margin: 0, fontStyle: 'italic', opacity: 0, transform: 'translateY(100%)' }}>
                {t.tyTitle}
            </h1>
          </div>
          ...
          <div style={{ overflow: 'hidden', marginBottom: '40px' }}>
            <p className="ty-text" style={{ fontFamily: '"Inter", sans-serif', fontSize: '1rem', letterSpacing: '3px', textTransform: 'uppercase', margin: 0, opacity: 0, transform: 'translateY(100%)' }}>
                {t.tySubtitle}
            </p>
          </div>

          <div className="ty-btn" style={{ opacity: 0, transform: 'translateY(20px)' }}>
            <button onClick={() => navigate('/')} style={{ background: accentColor, color: '#082019', border: 'none', padding: '15px 40px', fontFamily: '"Inter", sans-serif', fontSize: '0.8rem', letterSpacing: '2px', cursor: 'pointer', textTransform: 'uppercase', fontWeight: 'bold' }}>
                {t.tyBack}
            </button>
          </div>
      </div>
