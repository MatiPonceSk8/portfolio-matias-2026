import React from 'react';

const Navbar = ({ language, setLanguage }) => {
  
  const scrollToPosition = (percentage) => {
    const totalHeight = document.body.scrollHeight - window.innerHeight;
    const targetPosition = totalHeight * percentage;
    window.scrollTo({ top: targetPosition, behavior: 'smooth' });
  };

  const texts = {
    en: { items: ['HOME', 'PROJECTS', 'CONTACT'] },
    es: { items: ['INICIO', 'PROYECTOS', 'CONTACTO'] }
  };

  const t = texts[language] || texts.en;

  // Estilos comunes para los íconos SVG
  const iconStyle = {
    width: '20px',
    height: '20px',
    fill: 'currentColor', // Esto hace que tomen el color del texto (blanco/negro según el fondo)
    cursor: 'pointer',
    transition: 'opacity 0.3s'
  };

  return (
    <nav style={{
      position: 'fixed', top: 0, left: 0, width: '100%', padding: '20px 40px',
      display: 'flex', justifyContent: 'space-between', alignItems: 'center',
      zIndex: 100, boxSizing: 'border-box',
      mixBlendMode: 'difference', // La magia del color inverso
      color: '#EDE8E4' 
    }}>
      
      {/* LOGO */}
      <div style={{ fontFamily: '"Playfair Display", serif', fontSize: '1.5rem', fontWeight: 'bold', cursor: 'pointer' }} onClick={() => scrollToPosition(0)}>
        Matias<span style={{ fontFamily: '"Inter", sans-serif', fontSize: '0.9rem', opacity: 0.7 }}>.dev</span>
      </div>

      {/* LINKS CENTRALES */}
      <div style={{ display: 'flex', gap: '40px', fontFamily: '"Inter", sans-serif', fontSize: '0.8rem', letterSpacing: '2px', fontWeight: '500' }}>
        <span onClick={() => scrollToPosition(0)} style={{ cursor: 'pointer' }}>{t.items[0]}</span>
        <span onClick={() => scrollToPosition(0.36)} style={{ cursor: 'pointer' }}>{t.items[1]}</span> {/* Tu ajuste del 0.36 */}
        <span onClick={() => scrollToPosition(0.99)} style={{ cursor: 'pointer' }}>{t.items[2]}</span>
      </div>

      {/* DERECHA: IDIOMA + REDES */}
      <div style={{ display: 'flex', gap: '25px', alignItems: 'center' }}>
        
        {/* Idioma */}
        <div style={{ fontFamily: '"Inter", sans-serif', fontSize: '0.8rem', fontWeight: 'bold', cursor: 'pointer', marginRight: '10px' }}>
          <span onClick={() => setLanguage('en')} style={{ opacity: language === 'en' ? 1 : 0.4 }}>EN</span>
          <span style={{ margin: '0 5px', opacity: 0.4 }}>/</span>
          <span onClick={() => setLanguage('es')} style={{ opacity: language === 'es' ? 1 : 0.4 }}>ES</span>
        </div>

        {/* Separador sutil */}
        <div style={{ width: '1px', height: '15px', backgroundColor: 'currentColor', opacity: 0.3 }}></div>

        {/* GITHUB ICON */}
        <a href="https://github.com/TU_USUARIO" target="_blank" rel="noreferrer" style={{ color: 'inherit', display: 'flex' }}>
            <svg viewBox="0 0 24 24" style={iconStyle} onMouseEnter={(e) => e.target.style.opacity = 0.6} onMouseLeave={(e) => e.target.style.opacity = 1}>
                <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"/>
            </svg>
        </a>

        {/* LINKEDIN ICON */}
        <a href="https://linkedin.com/in/TU_USUARIO" target="_blank" rel="noreferrer" style={{ color: 'inherit', display: 'flex' }}>
            <svg viewBox="0 0 24 24" style={iconStyle} onMouseEnter={(e) => e.target.style.opacity = 0.6} onMouseLeave={(e) => e.target.style.opacity = 1}>
                <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/>
            </svg>
        </a>

      </div>
    </nav>
  );
};

export default Navbar;